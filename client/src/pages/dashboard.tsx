...
export default function Dashboard() {
  ...
  const assetConditions = assets.reduce(
    (acc, asset) => {
  ...
  const assetStatusDistribution = assets.reduce(
    (acc, asset) => {
  ...
  const expiringSoon = assets.filter((asset) => {
    ...
    const daysUntilExpiry =
      (warrantyDate.getTime() - currentDate.getTime()) / (1000 * 60 * 60 * 24);
  ...
  const assetHealthScore =
    totalAssets > 0
      ? Math.round(((totalAssets - maintenanceDue) / totalAssets) * 100)
      : 100;
  const cctvHealthScore =
    cctvSystems > 0 ? Math.round((cctvActive / cctvSystems) * 100) : 100;
  const biometricHealthScore =
    biometricSystems > 0
      ? Math.round((biometricActive / biometricSystems) * 100)
      : 100;
  const overallSystemHealth = Math.round(
    (assetHealthScore + cctvHealthScore + biometricHealthScore) / 3,
  );
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-white/70 mt-1">
            Centralized Master Data Management platform for enterprise resource
            lifecycle, workforce allocation, and operational intelligence.
          </p>
        </div>
      </div>
      ...
      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
        ...
        >
          ...
          <CardContent>
            ...
            <p className="text-xs text-white/60">
              Worth ₹{(totalAssetValue / 100000).toFixed(1)}L • {totalLocations}{" "}
              locations
            </p>
          </CardContent>
        </Card>
        ...
        >
          ...
          <CardContent>
            <div
              className="text-2xl font-bold text-white"
              data-testid="text-assigned-assets"
            >
              {assignmentRate}%
            </div>
        ...
        >
          ...
          <CardContent>
            ...
            <p className="text-xs text-white/60">
              Scheduled • {maintenanceInProgress} active •{" "}
        ...
        >
          ...
          <CardContent>
            <div className="flex items-center justify-between">
              ...
              <Badge
                variant={cctvHealthScore >= 90 ? "default" : "destructive"}
        ...
        >
          ...
          <CardContent>
            <div className="flex items-center justify-between">
              <div>
                <div className="text-2xl font-bold text-white">
                  {biometricActive}/{biometricSystems}
                </div>
                <p className="text-xs text-white/60">
                  {enrolledUsers} users enrolled
                </p>
              </div>
              <Badge
                variant={biometricHealthScore >= 90 ? "default" : "destructive"}
                className="bg-red-500/80 text-white border-0"
              >
        ...
        >
          ...
          <CardContent>
            <div className="space-y-2">
              <div>
                <div className="text-2xl font-bold text-white">
                  ₹{(maintenanceCosts / 100000).toFixed(1)}L
                </div>
      ...
      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
        ...
        <Card className="glass-card border-0">
          <CardHeader>
            ...
            <p className="text-xs text-white/50 mt-1">
              Equipment categorization across inventory
            </p>
          </CardHeader>
          <CardContent className="space-y-3">
            {Object.entries(assetTypeDistribution)
              ...
              .map(([type, count], index) => {
                ...
                const percentage =
                  totalAssets > 0 ? ((count as number) / totalAssets) * 100 : 0;
                  ...
                  >
                    <div className="flex items-center justify-between mb-1">
                      <div className="flex items-center gap-3">
                        <div
                          className={`w-10 h-10 ${colorClass} rounded-lg flex items-center justify-center group-hover:scale-110 transition-transform`}
                        >
                      ...
                      <span className="text-white/50 group-hover:text-white/90 transition-colors">
                        →
                      </span>
                    </div>
                  </div>
        ...
        <Card className="glass-card border-0">
          <CardHeader>
            ...
            <p className="text-xs text-white/50 mt-1">
              Operational state and availability metrics
            </p>
          </CardHeader>
          <CardContent className="space-y-4">
            {Object.entries(assetStatusDistribution).map(([status, count]) => {
              ...
              ] || { color: "bg-gray-400", icon: "⚪", label: status };
        ...
        <Card className="glass-card border-0">
          <CardHeader>
            ...
            <p className="text-xs text-white/50 mt-1">
              Workforce and equipment deployment analysis
            </p>
          </CardHeader>
          <CardContent className="space-y-4">
            <div className="grid grid-cols-2 gap-4">
              ...
              >
                ...
                <div className="text-xs text-purple-300 mt-1 flex items-center justify-between">
                  <span>
                    {((assignedAssets / totalAssets) * 100).toFixed(0)}%
                    utilization
                  </span>
                  <span className="text-white/50 group-hover:text-white/90 transition-colors">
                    →
              ...
              <div className="flex items-center justify-between">
                ...
                <div className="text-right flex flex-col items-end">
                  <div className="text-sm font-medium text-green-300">
                    {((availableAssets / totalAssets) * 100).toFixed(0)}%
                  </div>
                  <div className="text-xs text-white/50 group-hover:text-white/90 transition-colors">
                    View →
                  </div>
                </div>
              </div>
                ...
                >
                  ...
                  <div className="flex-1 space-y-1">
                    <p className="text-sm font-medium text-white/90">
                      {item.asset?.assetId} assigned to{" "}
                      {item.employee?.firstName} {item.employee?.lastName}
                    </p>
                    <div className="flex items-center gap-2 text-xs text-white/60">
                      <span>
                        {item.asset?.brand} {item.asset?.modelName}
                      </span>
                      <span>•</span>
                      <span>
              ...
              .map(([condition, count]) => {
                ...
                return (
                  <div key={condition} className="space-y-2">
                    <div className="flex items-center justify-between">
                      <span className="text-sm capitalize font-medium text-white/90">
                        {condition}
                      </span>
      ...
      <Card className="glass-card border-0">
        <CardHeader>
          ...
          <p className="text-sm text-white/70">
            Streamlined access to core MDM functions
          </p>
        </CardHeader>
        <CardContent>
