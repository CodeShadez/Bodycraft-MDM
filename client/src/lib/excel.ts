  ...
  static exportEmployees(
    ...
    const formattedData = employees.map((employee) => {
      ...
      const activeAssignment = assignments.find(
        (a) => a.employeeId === employee.id && !a.returnedDate,
      );
      const assignedAsset = activeAssignment
        ? assets.find((a) => a.assetId === activeAssignment.assetId)
        : null;
      ...
      return {
        ...
        "Assigned Asset": assignedAsset
          ? `${assignedAsset.assetId} - ${assignedAsset.brand} ${assignedAsset.modelName}`
          : "No Asset",
  ...
  static exportAssignments(
    ...
    const formattedData = assignments.map((assignment) => {
      ...
      return {
        ...
        "Returned Date": assignment.returnedDate
  ...
  static exportLocations(
    ...
    const formattedData = locations.map((location) => {
      ...
      return {
        ...
        "Available Assets": locationAssets.filter(
          (a) => a.status === "available",
        ).length,
        "Assigned Assets": locationAssets.filter((a) => a.status === "assigned")
          .length,
        "Maintenance Assets": locationAssets.filter(
          (a) => a.status === "maintenance",
        ).length,
        "Total Employees": locationEmployees.length,
        "Active Employees": locationEmployees.filter(
          (e) => e.status === "active",
        ).length,
  ...
  static exportCompliance(compliance: any[], assets: any[], locations: any[]) {
    const formattedData = compliance.map((record) => {
      const asset = record.assetId
        ? assets.find((a) => a.assetId === record.assetId)
        : null;
      const location = locations.find((l) => l.id === record.locationId);

      return {
        "Task Type":
          record.type.replace("_", " ").charAt(0).toUpperCase() +
          record.type.replace("_", " ").slice(1),
        Category:
          record.category.replace("_", " ").charAt(0).toUpperCase() +
          record.category.replace("_", " ").slice(1),
        Title: record.title,
        Description: record.description,
        "Related Asset": asset
...
export class ExcelImporter {
    ...
    data.forEach((row, index) => {
      ...
      const transformedRow = {
        ...
        joinDate: row["Join Date"]
          ? new Date(row["Join Date"]).toISOString()
          : null,
  ...
  static validateAssignmentData(data: any[]): {
    valid: any[];
    errors: string[];
  } {
    const valid: any[] = [];
  ...
  static validateMaintenanceData(data: any[]): {
    valid: any[];
    errors: string[];
  } {
    const valid: any[] = [];
