...
import { filterByUserLocation, canAccessLocation } from "./middleware/auth";
...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.get("/api/assets", requireAuth, async (req, res) => {
    try {
      ...
      const filteredAssets = filterByUserLocation(assets, req as Request);
  ...
  app.get("/api/assets/:id", requireAuth, async (req, res) => {
    try {
      ...
      if (!canAccessLocation(req as Request, asset.locationId)) {
  ...
  app.patch(
    "/api/assets/:id",
    requireAuth,
    requireRole(["super_admin", "admin", "location_user"]),
    async (req, res) => {
      try {
        ...
        if (
          getUserRole(req as Request) === "location_user" &&
          validatedAsset.locationId &&
          validatedAsset.locationId !== existingAsset.locationId
        ) {
          return res
            .status(403)
            .json({ error: "Cannot transfer asset to another location" });
        }
  ...
  app.get("/api/employees", requireAuth, async (req, res) => {
    try {
      ...
      const filteredEmployees = filterByUserLocation(employees, req as Request);
  ...
  app.get("/api/employees/:id", requireAuth, async (req, res) => {
    try {
      ...
      if (!canAccessLocation(req as Request, employee.locationId)) {
        return res
          .status(403)
          .json({ error: "Access denied to this employee" });
      }
  ...
  app.patch(
    ...
    async (req, res) => {
      try {
        const existingEmployee = await storage.getEmployee(
        ...
        const validatedEmployee = insertEmployeeSchema
          .partial()
          .parse(req.body);
        ...
        if (
          getUserRole(req as Request) === "location_user" &&
          validatedEmployee.locationId &&
          validatedEmployee.locationId !== existingEmployee.locationId
        ) {
          return res
            .status(403)
            .json({ error: "Cannot transfer employee to another location" });
        }
  ...
  app.patch(
    ...
    async (req, res) => {
      try {
        const validatedLocation = insertLocationSchema
          .partial()
          .parse(req.body);
    ...
    async (req, res) => {
      try {
        ...
        if (error instanceof z.ZodError) {
          return res
            .status(400)
            .json({ error: "Invalid department data", details: error.errors });
        }
  ...
  app.patch(
    ...
    async (req, res) => {
      try {
        const validatedDepartment = insertDepartmentSchema
          .partial()
          .parse(req.body);
    ...
    async (req, res) => {
      try {
        ...
        if (!department.isCustom) {
          return res
            .status(403)
            .json({ error: "Cannot delete predefined departments" });
        }
  ...
  app.get("/api/assignments", requireAuth, async (req, res) => {
    try {
      ...
      const employeeId = req.query.employeeId
        ...
        : undefined;
      const assignments = await storage.getAssignmentHistory(
        assetId,
        employeeId,
      );
      ...
      const accessibleAssignments = filteredAssignments
        .filter(({ asset }) =>
          canAccessLocation(req, asset?.locationId || null),
        )
  ...
  app.post(
    ...
    async (req, res) => {
      try {
        const validatedAssignment = insertAssetAssignmentHistorySchema.parse(
          req.body,
        );
        ...
        if (!canAccessLocation(req, asset?.locationId || null)) {
          return res
            .status(403)
            .json({ error: "Access denied to assign this asset" });
        }
  ...
  app.post(
    ...
    async (req, res) => {
      try {
        const validatedMaintenance = insertAssetMaintenanceSchema.parse(
          req.body,
        );
        ...
        const maintenance =
          await storage.createMaintenanceRecord(validatedMaintenance);
        res.status(201).json(maintenance);
      } catch (error) {
        if (error instanceof z.ZodError) {
          return res
            .status(400)
            .json({ error: "Invalid maintenance data", details: error.errors });
        }
  ...
  app.patch(
    ...
    async (req, res) => {
      try {
        ...
        const existing = allMaintenance.find(
        ...
        if (!existing) {
          return res
            .status(404)
            .json({ error: "Maintenance record not found" });
        }
        ...
        if (!canAccessLocation(req, asset?.locationId || null)) {
          return res
            .status(403)
            .json({ error: "Access denied to this maintenance record" });
        }

        const validatedMaintenance = insertAssetMaintenanceSchema
          .partial()
          .parse(req.body);

        // Prevent location_user from changing assetId (reassigning to different asset/location)
        if (
          req.session.role === "location_user" &&
          validatedMaintenance.assetId &&
          validatedMaintenance.assetId !== existing.assetId
        ) {
          return res
            .status(403)
            .json({
              error: "Cannot reassign maintenance record to a different asset",
            });
        }

        const maintenance = await storage.updateMaintenanceRecord(
          parseInt(req.params.id),
  ...
  app.delete(
    ...
    async (req, res) => {
      try {
        const success = await storage.deleteMaintenanceRecord(
          parseInt(req.params.id),
        );
        if (!success) {
          return res
  ...
  app.get("/api/cctv", requireAuth, async (req, res) => {
    try {
      ...
      const filteredSystems = filterByUserLocation(systems, req as Request);
    ...
    async (req, res) => {
      try {
        ...
        if (error instanceof z.ZodError) {
          return res
            .status(400)
            .json({ error: "Invalid CCTV system data", details: error.errors });
        }
  ...
  app.patch(
    ...
    async (req, res) => {
      try {
        ...
        const existing = allSystems.find(
        ...
        // Check location access BEFORE updating
        if (!canAccessLocation(req as Request, existing.locationId)) {
          return res
            .status(403)
            .json({ error: "Access denied to this CCTV system" });
        }

        const validatedSystem = insertCctvSystemSchema
          .partial()
          .parse(req.body);
        ...
        if (
          req.session.role === "location_user" &&
          validatedSystem.locationId &&
          validatedSystem.locationId !== existing.locationId
        ) {
          return res
            .status(403)
            .json({ error: "Cannot transfer CCTV system to another location" });
        }

        const system = await storage.updateCctvSystem(
          parseInt(req.params.id),
          validatedSystem,
        );
  ...
  app.get("/api/cctv/:id/snapshot", requireAuth, async (req, res) => {
    try {
      ...
      if (!canAccessLocation(req as Request, system.locationId)) {
        return res
          .status(403)
          .json({ error: "Access denied to this CCTV system" });
      }
      ...
      if (!system.ipAddress || !system.username || !system.passwordHash) {
        return res
          .status(400)
          .json({ error: "CCTV system not configured with credentials" });
      }
  ...
  app.get(
    "/api/cctv/:id/recordings/playback-url",
    requireAuth,
    async (req, res) => {
      try {
        const { HikvisionClient } = await import("./hikvision-client");
      ...
      try {
        ...
        if (error instanceof z.ZodError) {
          return res
            .status(400)
            .json({
              error: "Invalid biometric system data",
  ...
  app.patch(
    ...
    async (req, res) => {
      try {
        const validatedSystem = insertBiometricSystemSchema
          .partial()
          .parse(req.body);
        const system = await storage.updateBiometricSystem(
          parseInt(req.params.id),
          validatedSystem,
        );
        if (!system) {
  ...
  app.delete(
    ...
    async (req, res) => {
      try {
        const success = await storage.deleteBiometricSystem(
          parseInt(req.params.id),
        );
        if (!success) {
          return res.status(404).json({ error: "Biometric system not found" });
    ...
    async (req, res) => {
      try {
        ...
        if (error instanceof z.ZodError) {
          return res
            .status(400)
            .json({ error: "Invalid backup data", details: error.errors });
        }
  ...
  app.patch(
    ...
    async (req, res) => {
      try {
        ...
        const backup = await storage.updateBackup(
          parseInt(req.params.id),
          validatedBackup,
        );
        if (!backup) {
      ...
      const templates = [
        {
          ...
          description:
            "Complete inventory of all assets across locations with current status, condition, and assignment details",
          category: "assets",
          fields: [
            "assetId",
            "assetType",
        ...
        },
        {
          ...
          description:
            "Comprehensive maintenance activities, costs, and schedules with vendor performance analysis",
        ...
        },
        {
          ...
          description:
            "Historical tracking of asset assignments with employee details and assignment duration analysis",
        ...
        },
        {
          ...
          description:
            "Multi-location performance analytics with asset distribution, utilization rates, and cost analysis",
          category: "locations",
          fields: [
            "outletName",
            "city",
        ...
        },
        {
          ...
          description:
            "Comprehensive compliance status across backup verification, security audits, and policy adherence",
          category: "compliance",
          fields: [
            "locationId",
            "backupType",
      ...
      switch (templateId) {
        case "asset-inventory":
          ...
          if (filters?.assetType && filters.assetType !== "all") {
            data = data.filter(
              (asset: any) => asset.assetType === filters.assetType,
            );
          }
        ...
        case "maintenance-summary":
          ...
          if (filters?.status && filters.status !== "all") {
            data = data.filter(
              (maintenance: any) => maintenance.status === filters.status,
            );
          }
      ...
      const customReports = [
        {
          ...
          description:
            "Monthly tracking of laptop assignments for JP Nagar location",
          entity: "assignments",
    ...
    async (req, res) => {
      try {
        ...
        if (!name || !entity || !fields) {
          return res
            .status(400)
            .json({ error: "Name, entity, and fields are required" });
        }
  ...
  app.post(
    "/api/reports/custom/:id/generate",
    requireAuth,
    async (req, res) => {
      try {
        const { id } = req.params;
  ...
  app.get(
    ...
    async (req, res) => {
      try {
        ...
        const predictions = await storage.getPredictiveMaintenance(
          locationId,
          assetType,
          riskLevel,
        );
        ...
        res
          .status(500)
          .json({ message: "Failed to fetch predictive maintenance data" });
      }
    },
  );
  ...
  app.get(
    "/api/analytics/utilization-optimization",
    requireAuth,
    async (req, res) => {
      try {
        ...
        res
          .status(500)
          .json({ message: "Failed to fetch utilization optimization data" });
      }
    },
  );
  ...
  app.get(
    "/api/locations/performance-analytics",
    requireAuth,
    async (req, res) => {
      try {
        ...
        res
          .status(500)
          .json({ message: "Failed to fetch location performance analytics" });
      }
    },
  );
  ...
  app.post("/api/locations/transfer-asset", requireAuth, async (req, res) => {
    try {
      ...
      const { assetId, fromLocationId, toLocationId, transferReason } =
        validation.data;
      ...
      if (asset.status === "assigned") {
        return res
          ...
          .json({
            message:
              "Cannot transfer an assigned asset. Please return it first.",
          });
      }
  ...
  app.get("/api/dashboard/real-time-data", requireAuth, async (req, res) => {
    try {
      ...
      res
        .status(500)
        .json({ message: "Failed to fetch real-time dashboard data" });
    }
  });
    ...
    try {
      ...
      if (!["assets", "maintenance", "compliance"].includes(metric)) {
        return res
          .status(400)
          .json({
            message: "Invalid metric. Use: assets, maintenance, or compliance",
          });
      }
    ...
    async (req, res) => {
      try {
        ...
        if (updateData.status && req.session.role !== "super_admin") {
          return res
            .status(403)
            .json({ message: "Only super admins can change user status" });
        }
        ...
        if (req.session.role === "admin") {
          ...
          if (targetUser.role === "super_admin") {
            return res
              .status(403)
              .json({
                message: "Admins cannot reset passwords for super admins",
              });
          }
        }
      ...
      const accessibleRequests = await Promise.all(
        requests.map(async (request) => {
          const currentValue = request.currentValue
            ? JSON.parse(request.currentValue)
            : {};
  ...
  app.post(
    "/api/approvals",
    requireAuth,
    requireRole(["location_user", "admin", "super_admin"]),
    async (req, res) => {
      try {
        const validation = z
          .object({
        ...
        if (entityType === "asset") {
          ...
          if (!canAccessLocation(req, asset.locationId)) {
            return res
              .status(403)
              .json({ error: "Access denied to this asset" });
          }
          ...
          if (
            ...
            if (!asset) {
              return res
                .status(404)
                .json({
                  error: "Asset no longer exists and cannot be transferred",
                });
            }
            ...
            request.requestType === "asset_assignment" &&
            request.entityType === "asset"
          ) {
            const asset = await storage.getAsset(request.entityId);
            if (!asset) {
              return res
                .status(404)
                .json({
                  error: "Asset no longer exists and cannot be assigned",
                });
            }
            ...
            if (!employee) {
              return res
                .status(404)
                .json({ error: "Employee no longer exists for assignment" });
            }
            ...
            request.requestType === "employee_transfer" &&
            ...
            if (!employee) {
              return res
                .status(404)
                .json({
                  error: "Employee no longer exists and cannot be transferred",
                });
            }
            await storage.updateEmployee(parseInt(request.entityId), {
              locationId: newValue.locationId,
            });
          }
    ...
    async (req, res) => {
      try {
        ...
        res
          .status(500)
          .json({
            error: "Failed to cleanup sample data",
        ...
        const demoInvoices: InsertInvoice[] = [
          {
            ...
            paymentStatus: "paid",
            description:
              "Laptop screen replacement and motherboard repair for Store #12",
          },
          {
          ...
          },
          {
            ...
            paymentStatus: "unpaid",
            description:
              "CCTV camera repair and networking equipment maintenance",
          },
          {
          ...
          },
          {
            ...
            paymentStatus: "paid",
            description:
              "Quarterly maintenance contract for all IT equipment at HQ",
          },
          {
  ...
  app.put("/api/compliance/tasks/:id", requireAuth, async (req, res) => {
    try {
      ...
      if (
        req.session.role === "location_user" &&
        req.session.locationId !== existingTask.locationId
      ) {
  ...
  app.post(
    "/api/compliance/tasks/:id/evidence",
    requireAuth,
    async (req, res) => {
      try {
        ...
        if (
          req.session.role === "location_user" &&
          req.session.locationId !== task.locationId
        ) {
          return res.status(403).json({ error: "Access denied to this task" });
        }
  ...
  app.get(
    "/api/compliance/tasks/:id/audit-trail",
    requireAuth,
    async (req, res) => {
      try {
        const taskId = parseInt(req.params.id);
  ...
  app.get("/api/compliance/analytics", requireAuth, async (req, res) => {
    try {
      const locationId =
        req.session.role === "location_user"
          ? (req.session.locationId ?? undefined)
          : undefined;
      ...
      const upcomingTasks = tasks
        .filter((t) => {
          ...
          return (
            t.status === "pending" && dueDate >= today && dueDate <= next7Days
          );
        })
        .sort(
          (a, b) =>
    ...
    async (req, res) => {
      try {
        ...
        res.json({
          success: true,
          message: locationId
            ? `Automation run completed for location ${locationId}`
            : "Automation run completed for all locations",
        });
      } catch (error) {
  ...
  app.get(
    "/api/compliance/automation/summary",
    requireAuth,
    async (req, res) => {
      try {
  ...
  app.get(
    "/api/compliance/predictive-alerts",
    requireAuth,
    async (req, res) => {
      try {
        const locationId = req.query.locationId
  ...
  app.post(
    "/api/compliance/tasks/auto-generate",
    requireAuth,
    requireRole(["super_admin", "admin"]),
    async (req, res) => {
      try {
        ...
        res.json({
          success: true,
          message: locationId
            ? `Tasks auto-generated for location ${locationId}`
            : "Tasks auto-generated for all locations",
        });
      } catch (error) {
  ...
  app.get("/api/backups/verification", requireAuth, async (req, res) => {
    try {
      ...
      const verifications =
        await backupVerifier.getVerificationHistory(assetId);
  ...
  app.post(
    "/api/backups/verification/trigger",
    requireAuth,
    requireRole(["super_admin", "admin"]),
    async (req, res) => {
      try {
        ...
        res.json({
          success: true,
          message: locationId
            ? `Backup verification completed for location ${locationId}`
            : "Backup verification completed for all locations",
        });
      } catch (error) {
