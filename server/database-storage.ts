...
import {
  ...
  // Advanced/AI/Compliance/Automation types
  type Invoice,
  type InsertInvoice,
  ...
  insertComplianceSignalSchema,
  insertComplianceRiskScoreSchema,
  insertAutomationRunSchema,
  insertAiRecommendationSchema,
  insertBackupVerificationSchema,
  insertComplianceAssignmentQueueSchema,
  insertComplianceTaskSchema,
  insertComplianceEvidenceSchema,
  insertComplianceAuditTrailSchema,
  insertAssetTransferSchema,
  passwordResetSchema,
...
export class DatabaseStorage implements IStorage {
  ...
  async createDepartment(department: InsertDepartment): Promise<Department> {
    const result = await db
      ...
      .values({
        ...department,
        isActive:
          department.isActive !== undefined ? department.isActive : true,
        isCustom:
          department.isCustom !== undefined ? department.isCustom : false,
        createdAt: new Date().toISOString(),
      })
      .returning();
    return result[0];
  ...
  async getAssignmentHistory(
    ...
    let query: any = db.select().from(assetAssignmentHistory);
    ...
    if (assetId && employeeId) {
      ...
      query = query.where(eq(assetAssignmentHistory.assetId, assetId)) as any;
    } else if (employeeId) {
      query = query.where(eq(assetAssignmentHistory.employeeId, employeeId)) as any;
    }
  ...
  async getMaintenanceRecords(assetId?: string): Promise<AssetMaintenance[]> {
    let query: any = db.select().from(assetMaintenance);

    if (assetId) {
      query = query.where(eq(assetMaintenance.assetId, assetId)) as any;
    }
  ...
  async getAllCctvSystems(): Promise<CctvSystem[]> {
    return await db
      .select()
      .from(cctvSystems)
      .orderBy(desc(cctvSystems.createdAt));
  }
  ...
  async getAllBiometricSystems(): Promise<BiometricSystem[]> {
    return await db
      .select()
      .from(biometricSystems)
      .orderBy(desc(biometricSystems.createdAt));
  }
  ...
  async getBackups(assetId?: string): Promise<Backup[]> {
    let query: any = db.select().from(backups);

    if (assetId) {
      query = query.where(eq(backups.assetId, assetId)) as any;
    }
  ...
  async getDashboardStats() {
    ...
    const [
      ...
      db
        .select()
        .from(biometricSystems)
        .where(eq(biometricSystems.status, "online")),
    ]);
    ...
    return {
      ...
      utilizationRate:
        totalAssets.length > 0
          ? Math.round((assignedAssets.length / totalAssets.length) * 100)
          : 0,
  ...
  async getRecentActivities(limit: number = 10) {
    // Drizzle ORM does not support selecting a string literal as a field directly, so map after query
    const results = await db
      ...
      .limit(limit);
    return results.map((row) => ({ ...row, type: "assignment" }));
  }
  ...
  async createApprovalRequest(
    ...
    const {
      requestType, entityType, entityId, currentValue, newValue, reason, requestedBy, requestedAt, currentApprovalLevel, requiredApprovalLevels, completedAt, completedBy, createdAt, updatedAt
    } = request;
    const result = await db
      ...
      .values({
        ...
        reason,
        requestedBy,
        requestedAt: requestedAt ? requestedAt : new Date().toISOString(),
        currentApprovalLevel: currentApprovalLevel || 1,
        requiredApprovalLevels: requiredApprovalLevels || 1,
        completedAt,
        completedBy,
  ...
  async getComplianceTasks(filters?: {
    ...
    const baseQuery = db
      ...
      .leftJoin(
        assignedToUser,
        eq(complianceTasks.assignedTo, assignedToUser.id),
      )
    ...
    if (taskIds.length > 0) {
      ...
      for (const taskId of taskIds) {
        ...
        evidenceMap[taskId] = taskEvidence.map((e) => e.fileUrl).filter((f): f is string => f !== null);
      }
    }
    ...
    return results.map((row: any) => {
      ...
      return {
        ...
        evidenceFiles: evidenceMap[row.task.id] || [],
        isOverdue,
  ...
  async createComplianceTask(
    task: InsertComplianceTask,
  ): Promise<ComplianceTask> {
    // Only include properties that exist in the schema
    const {
      id, title, description, taskType, status, dueDate, completedAt, assignedTo, createdBy, locationId, relatedAssetId
    } = task;
    const result = await db
  ...
  async updateComplianceTask(
    id: number,
    task: Partial<InsertComplianceTask>,
  ): Promise<ComplianceTask | undefined> {
    // Only include properties that exist in the schema
    const {
      title, description, taskType, status, dueDate, completedAt, assignedTo, createdBy, locationId, relatedAssetId
    } = task;
    const result = await db
  ...
  async getComplianceDashboardStats(locationId?: number): Promise<any> {
    ...
    if (locationId) {
      tasksQuery = tasksQuery.where(
        eq(complianceTasks.locationId, locationId),
      ) as any;
    }
  ...
  async uploadComplianceEvidence(
    evidence: InsertComplianceEvidence,
  ): Promise<ComplianceEvidence> {
    // Only include properties that exist in the schema
    const {
      taskId, evidenceType, fileUrl, fileName, fileType, fileSize, uploadedBy
    } = evidence;
    const result = await db
  ...
  async createComplianceAuditTrail(
    trail: InsertComplianceAuditTrail,
  ): Promise<ComplianceAuditTrail> {
    // Only include properties that exist in the schema
    const {
      taskId, action, performedBy, comments
    } = trail;
    const result = await db
  ...
  async getPredictiveMaintenance(
    ...
    const predictions = await Promise.all(
      assignedAssets.map(async (row: any) => {
        ...
        const lastMaintenance = maintenanceHistory.sort(
          (a, b) =>
        ...
        const lastMaintenanceDate = lastMaintenance
          ? lastMaintenance.completedDate || lastMaintenance.scheduledDate
          : null;
  ...
  async getUtilizationOptimization(): Promise<any> {
    ...
    const locationStats = await Promise.all(
      allLocations.map(async (location) => {
        ...
        const assignedAssets = totalAssets.filter(
          (a) => a.status === "assigned",
        );
        const availableAssets = totalAssets.filter(
          (a) => a.status === "available",
        );
        const totalEmployees = await db
          .select()
          .from(employees)
          .where(eq(employees.locationId, location.id));

        const utilizationRate =
          totalAssets.length > 0
            ? Math.round((assignedAssets.length / totalAssets.length) * 100)
            : 0;
        const assetsPerEmployee =
          totalEmployees.length > 0
            ? Number((assignedAssets.length / totalEmployees.length).toFixed(2))
            : 0;

        // Calculate efficiency score (combination of utilization and assets per employee)
        const efficiencyScore = Math.round(
          utilizationRate * 0.7 + Math.min(assetsPerEmployee * 20, 30),
        );
    ...
    return {
      ...
      summary: {
        ...
        avgUtilization: Math.round(
          locationStats.reduce((sum, l) => sum + l.utilizationRate, 0) /
            locationStats.length,
        ),
        avgEfficiencyScore: Math.round(
          locationStats.reduce((sum, l) => sum + l.efficiencyScore, 0) /
            locationStats.length,
        ),
      },
    };
    ...
    const performance = await Promise.all(
      allLocations.map(async (location) => {
        ...
        const totalMaintenanceCost = maintenanceRecords.reduce(
          (sum, r) => sum + (Number(r.asset_maintenance.cost) || 0),
          0,
        );

        const locationComplianceTasks = await db
          .select()
          .from(complianceTasks)
          .where(eq(complianceTasks.locationId, location.id));

        const completedCompliance = locationComplianceTasks.filter(
          (t: any) => t.status === "completed",
        ).length;
        // avgComplianceScore removed: not in schema

        const utilizationRate =
          totalAssets.length > 0
            ? Math.round((activeAssets.length / totalAssets.length) * 100)
            : 0;
        const complianceRate =
          locationComplianceTasks.length > 0
            ? Math.round(
                (completedCompliance / locationComplianceTasks.length) * 100,
              )
            : 0;
        const assetPerEmployee =
          activeEmployees.length > 0
            ? Number((activeAssets.length / activeEmployees.length).toFixed(2))
            : 0;
        const avgMaintenanceCost =
          totalMaintenance > 0
            ? Number((totalMaintenanceCost / totalMaintenance).toFixed(2))
            : 0;
  ...
  async getRealTimeDashboardData(): Promise<any> {
    ...
    const retiredAssets = allAssets.filter(
      (a) => a.status === "retired",
    ).length;
    const utilizationRate =
    ...
    const completedMaintenance = allMaintenance.filter(
      (m) => m.completedDate,
    ).length;
    const scheduledMaintenance = allMaintenance.filter(
      (m) => m.scheduledDate && !m.completedDate,
    ).length;
    ...
    const avgMaintenanceCost =
      allMaintenance.length > 0
        ? Number(
            (
              allMaintenance.reduce((sum, m) => sum + Number(m.cost || 0), 0) /
    ...
    const maintenanceCompletionRate =
      allMaintenance.length > 0
        ? Math.round((completedMaintenance / allMaintenance.length) * 100)
        : 0;

    // Compliance stats
    const allComplianceTasks = await db.select().from(complianceTasks);
    const completedCompliance = allComplianceTasks.filter(
      (t) => t.status === "completed",
    ).length;
    const overdueCompliance = allComplianceTasks.filter(
      (t) => t.status === "pending" && t.dueDate && new Date(t.dueDate) < new Date(),
    ).length;
    // avgComplianceScore removed: not in schema
    const complianceCompletionRate =
      allComplianceTasks.length > 0
        ? Math.round((completedCompliance / allComplianceTasks.length) * 100)
        : 0;
    ...
    const recentActivities = [
      ...
      {
        type: "maintenance",
        entity: "Maintenance completed for BFC002",
        date: new Date(),
      },
      {
        type: "compliance",
        entity: "Compliance task updated",
        date: new Date(),
      },
    ].slice(0, 10);
  ...
  async getDashboardTrends(metric: string, period: string): Promise<any[]> {
    ...
    // This method should return an array of trend data objects.
    // Implement your trend data logic here as needed.
    return [];
  }

  // ==================== AI COMPLIANCE AUTOMATION METHODS ====================

  async createComplianceSignal(
    signal: InsertComplianceSignal,
  ): Promise<ComplianceSignal> {
    const result = await db
      .insert(complianceSignals)
      .values(signal)
      .returning();
    return result[0];
  }

  async getActiveComplianceSignals(filters?: {
    locationId?: number;
    severity?: string;
  }): Promise<ComplianceSignal[]> {
  ...
  async createComplianceRiskScore(
    score: InsertComplianceRiskScore,
  ): Promise<ComplianceRiskScore> {
    const result = await db
      .insert(complianceRiskScores)
      .values(score)
      .returning();
    return result[0];
  }

  async getLatestRiskScores(filters?: {
    locationId?: number;
    assetId?: string;
  }): Promise<ComplianceRiskScore[]> {
  ...
  // Helper for gte (greater than or equal) for Drizzle ORM
  ...
  async getAutomationRunSummary(): Promise<{
    ...
    const runs = await db
      .select()
      .from(automationRuns)
      .where(this.gte(automationRuns.startedAt, thirtyDaysAgo.toISOString()));

    const total = runs.length;
    const successful = runs.filter((r) => r.status === "completed").length;
    const failed = runs.filter((r) => r.status === "failed").length;
    const successRate = total > 0 ? Math.round((successful / total) * 100) : 0;
    const totalTasksGenerated = runs.reduce((sum, r) => sum + (r.tasksGenerated || 0), 0);
    const totalRisksDetected = runs.reduce((sum, r) => sum + (r.risksDetected || 0), 0);
    const avgExecutionTime = runs.length > 0 ? Math.round(runs.reduce((sum, r) => sum + (r.executionTimeMs || 0), 0) / runs.length) : 0;
    const lastRun = runs.length > 0 ? runs[0] as AutomationRun : null;
  ...
  async getBackupVerifications(filters?: {
    assetId?: string;
    status?: string;
  }): Promise<BackupVerification[]> {
    ...
    if (filters.status) {
      conditions.push(
        eq(backupVerification.verificationStatus, filters.status),
      );
    }
  ...
  async createComplianceAssignment(
    assignment: InsertComplianceAssignmentQueue,
  ): Promise<ComplianceAssignmentQueue> {
    const result = await db
      .insert(complianceAssignmentQueue)
      .values(assignment)
      .returning();
    return result[0];
  }

  async getComplianceAssignments(filters?: {
    userId?: number;
    status?: string;
  }): Promise<ComplianceAssignmentQueue[]> {
