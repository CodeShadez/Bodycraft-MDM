...
export class BackupVerifier {
  ...
  async runVerification(locationId?: number): Promise<void> {
    console.log(
      `Starting backup verification for location ${locationId || "all"}`,
    );
    ...
    for (const asset of filteredAssets) {
      try {
        ...
        const isBackupEligible =
          asset.assetType !== "monitor" && asset.assetType !== "keyboard";
        ...
        console.error(
          `Failed to verify backup for asset ${asset.assetId}:`,
          error,
        );
      }
    }
    ...
    console.log(
      `Backup verification completed for ${filteredAssets.length} assets`,
    );
  }
  ...
  private async checkBackupStatus(asset: any): Promise<BackupCheckResult> {
    ...
    const isBackupConfigured =
      asset.assetType !== "monitor" && asset.assetType !== "keyboard"; // Only backup servers/laptops
    ...
    if (hasBackupIssue) {
      return {
        ...
        lastBackupDate: new Date(
          now.getTime() - backupAge * 24 * 60 * 60 * 1000,
        ),
  ...
  private async createBackupSignal(
    ...
    const signal: InsertComplianceSignal = {
      ...
      signalType:
        result.backupStatus === "missing" ? "backup_missing" : "backup_failure",
  ...
  private async createBackupTask(
    ...
    dueDate.setDate(
      dueDate.getDate() + (result.backupStatus === "missing" ? 3 : 1),
    );
